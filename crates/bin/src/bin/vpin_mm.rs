...
use nautilus_bin::strategy::vpin_mm::{config::VpinMarketMakerConfig, strategy::VpinMarketMaker};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cfg = Config::load("config.toml".to_string())?
        .vpin_mm
        .expect("config.toml missing [vpin_mm] section");
    ...
    let config = VpinMarketMakerConfig::builder()
        ...
        .vpin_spread_alpha(cfg.vpin_spread_alpha)
        .vpin_pull_threshold(cfg.vpin_pull_threshold)
        .bucket_target_volume(cfg.bucket_target_volume)
        .timer_interval_secs(cfg.timer_interval_secs)
        .build();
    ...
    match &cfg.execution_environment {
        Environment::Backtest => {
            ...
            node.build()?;
            {
                let engine = node.get_engine_mut("vpin-mm-backtest").unwrap();

                let strategy = VpinMarketMaker::new(config);
